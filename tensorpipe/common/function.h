...
//inline std::atomic<Storage*> freeList = nullptr;
//inline bool freeListDead = false;
...
//  Storage* next = freeList.load(std::memory_order_relaxed);
//  do {
//    if (!next) {
//      return allocStorage(n);
//    }
//  } while (!freeList.compare_exchange_weak(next, next->next));
//  return next;
//}
...
//  do {
//    s->next = next;
//  } while (!freeList.compare_exchange_weak(next, s));
//}
