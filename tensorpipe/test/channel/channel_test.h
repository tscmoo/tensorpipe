...
template <>
class DataWrapper<rpc_tensorpipe::CpuBuffer> {
 public:
  explicit DataWrapper(size_t length) : vector_(length) {}

  explicit DataWrapper(std::vector<uint8_t> v) : vector_(v) {}

  rpc_tensorpipe::CpuBuffer buffer() const {
    return rpc_tensorpipe::CpuBuffer{const_cast<uint8_t*>(vector_.data()),
                                 vector_.size()};
  }
...
template <>
class DataWrapper<rpc_tensorpipe::CudaBuffer> {
 public:
  // Non-copyable.
  ...
  rpc_tensorpipe::CudaBuffer buffer() const {
    return rpc_tensorpipe::CudaBuffer{cudaPtr_, length_, stream_};
  }
...
class ChannelTestHelper {
 public:
  ...
  virtual std::shared_ptr<rpc_tensorpipe::channel::Context<TBuffer>> makeContext(
      std::string id) = 0;
...
[[nodiscard]] std::pair<
    std::future<
        std::tuple<rpc_tensorpipe::Error, rpc_tensorpipe::channel::TDescriptor>>,
    std::future<rpc_tensorpipe::Error>>
sendWithFuture(
    std::shared_ptr<rpc_tensorpipe::channel::Channel<TBuffer>> channel,
    TBuffer buffer) {
  auto descriptorPromise = std::make_shared<
      std::promise<std::tuple<rpc_tensorpipe::Error, std::string>>>();
  auto promise = std::make_shared<std::promise<rpc_tensorpipe::Error>>();
  ...
  channel->send(
      ...
      [promise{std::move(promise)}](const rpc_tensorpipe::Error& error) {
...
template <typename TBuffer>
[[nodiscard]] std::future<rpc_tensorpipe::Error> recvWithFuture(
    std::shared_ptr<rpc_tensorpipe::channel::Channel<TBuffer>> channel,
    rpc_tensorpipe::channel::TDescriptor descriptor,
    TBuffer buffer) {
  auto promise = std::make_shared<std::promise<rpc_tensorpipe::Error>>();
  auto future = promise->get_future();
...
class ClientServerChannelTestCase : public ChannelTestCase<TBuffer> {
 public:
  void run(ChannelTestHelper<TBuffer>* helper) override {
    ...
    peers_->spawn(
        [&] {
          auto context = std::make_shared<rpc_tensorpipe::transport::uv::Context>();
          context->setId("server_harness");

          auto listener = context->listen(addr);

          std::promise<std::shared_ptr<rpc_tensorpipe::transport::Connection>>
              connectionProm;
          listener->accept(
              [&](const rpc_tensorpipe::Error& error,
                  std::shared_ptr<rpc_tensorpipe::transport::Connection>
                      connection) {
  ...
  virtual void server(std::shared_ptr<rpc_tensorpipe::transport::Connection>) {}
  virtual void client(std::shared_ptr<rpc_tensorpipe::transport::Connection>) {}
...
class CpuChannelTestSuite : public ::testing::TestWithParam<
                                ChannelTestHelper<rpc_tensorpipe::CpuBuffer>*> {};

#if TENSORPIPE_SUPPORTS_CUDA
class CudaChannelTestSuite : public ::testing::TestWithParam<
                                 ChannelTestHelper<rpc_tensorpipe::CudaBuffer>*> {};
#endif // TENSORPIPE_SUPPORTS_CUDA

#define _CHANNEL_TEST(type, suite, name)    \
  TEST_P(suite, name) {                     \
    name##Test<rpc_tensorpipe::type##Buffer> t; \
    t.run(GetParam());                      \
  }
