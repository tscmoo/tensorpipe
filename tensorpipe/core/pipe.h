...
class Pipe final {
 ...
 public:
  ...
  using read_descriptor_callback_fn =
      Function<void(const Error&, MessageHandle)>;

  void readDescriptor(read_descriptor_callback_fn);

  using read_callback_fn = Function<void(const Error&, MessageHandle)>;

  void read(MessageHandle, read_callback_fn);

  using write_callback_fn = Function<void(const Error&, MessageHandle)>;
